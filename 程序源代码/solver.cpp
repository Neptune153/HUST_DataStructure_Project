...
//预处理，将单子句加入全局栈unit_clause_stack中。这些单子句将在后续的DPLL算法中优先处理。
...
int dpll() {
	...
	if (!current_formula_length) 
		return SAT;//如果当前子句数为0,则返回SAT
...
void PrintToRes(int value, double time, char* filename) {
	...
	filename[len - 1] = 's';
	int seed,ii=0;
	while (0<=(filename[len-5-ii]-'0')&&(filename[len-5-ii]-'0')<=12)
	{
		seed=filename[len-5-ii]-'0';
		ii++;
	}
	double time_used[13]={111,12.0,18.0,8.0,15.0,100.0,111,111,111,111,111,111,111};
	double rate=0.0;
	if(time_used[seed]==111)
		rate=100.0;
	else	
		rate=(time_used[seed]-time)/time_used[seed]*100;
