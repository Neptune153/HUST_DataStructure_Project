...
int createfinalsudoku(int i, int j){
	...
	for (int k = 1; k <= 9; ++k){
		...
		if(type&&i==j){
			//检查整个棋盘的主副两条对角线是否出现过数字k
			for(int s=1;s<i;s++)
				if(sudokuterminal[s][s]==k)
					type=false;
		}
		if(type&&i+j==10){
			//检查整个棋盘的主副两条对角线是否出现过数字k
			for(int s=1;s<i;s++)
				if(sudokuterminal[s][9-s+1]==k)
					type=false;
		}
		if (type){
...
for (int k = 1; k <= 9; k++) {
    for (int i = 0; i < 9; i++) {
        for (int j = i + 1; j < 9; j++) {
            fprintf(fp, "-%d -%d 0\n", Location(i, 8 - i, k), Location(j, 8 - j, k));
        }
    }
}
*/
